...
edit_distance_df = pd.read_csv('edit_distance.csv')
...
val = valid_df.loc[:]
error = error_df.loc[:]
edit_distance = edit_distance_df.iloc[:]
train_set = MyDataset(train)
val_set = MyDataset(val)
error_set = MyDataset(error)
edit_distance_set = MyDataset(edit_distance)
train_loader = DataLoader(train_set, batch_size=CFG['train_bs'], collate_fn=collate_fn, shuffle=True,
                          num_workers=CFG['num_workers'])
error_loader = DataLoader(error_set, batch_size=CFG['train_bs'], collate_fn=collate_fn, shuffle=True,
                          num_workers=CFG['num_workers'])
edit_distance_loader = DataLoader(edit_distance_set, batch_size=CFG['train_bs'], collate_fn=collate_fn, shuffle=True,
                          num_workers=CFG['num_workers'])
...
for epoch in range(CFG['epochs']):
    ...
    time.sleep(0.2)
    if epoch == 0:
        train_loss, train_acc = train_model(model, fgm, pgd, edit_distance_loader)
    else:
        train_loss, train_acc = train_model(model, fgm, pgd, error_loader)
    train_loss, train_acc = train_model(model, fgm, pgd, train_loader)
    val_loss, val_acc, F1 = test_model(model, val_loader)
    if F1 > best_F1:
        best_F1 = F1
        torch.save(model.state_dict(), '8_mrcmac_error_fgm.pt')

model.load_state_dict(torch.load('8_mrcmac_error_fgm.pt'.format(epoch)))
optimizer = AdamW(model.parameters(), lr=2e-6, weight_decay=CFG['weight_decay'])  # AdamW优化器
train_loss, train_acc = train_model(model, fgm, pgd, val_loader)
torch.save(model.state_dict(), '8_mrcmac_error_fgm.pt')