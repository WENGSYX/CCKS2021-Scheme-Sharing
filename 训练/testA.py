...
y1 = np.load('3_mac_fgm.npy')
y2 = np.load('2_ro_fgm.npy')
y3 = np.load('6_mrcmac_error_fgm.npy')
y4 = np.load('7_mrcmac_error_fgm.npy')
y5 = np.load('8_mrcmac_error_fgm.npy')
jia = np.ones((5000, 2)) * np.array([0, 4.7])
...
for i in range(len(test_df)):
    ...
    t.append({'Label': y[i], 'Docid': item['docid'], 'Question': item['question'], 'Description': item['description'],
              'Answer': item['answer']})
t = pd.DataFrame(t)
t.to_csv('白[MASK]_test_resultA.csv')